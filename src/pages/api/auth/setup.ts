...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (isSetupComplete()) {
      return new Response(JSON.stringify({ success: false, message: "Setup already completed" }), {
        status: 400,
    ...
    switch (swarmAction) {
      ...
      case "join":
        if (!joinToken) {
          return new Response(JSON.stringify({ success: false, message: "Join token is required" }), {
            status: 400,
            headers: { "Content-Type": "application/json" },
          })
        }
